...
export default function ShippedOrdersPage() {
    ...
    const { data: ordersData, isLoading } = useQuery({
        queryKey: ["shippedOrders", debouncedSearch, page, limit, dateRange],
        queryFn: async () => {
            const params = new URLSearchParams();
            if (debouncedSearch) params.append("search", debouncedSearch);
    ...
    return (
        <div className="flex flex-col min-h-screen bg-[#f9fafb] p-6 lg:p-8 space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-lg font-bold text-gray-800 tracking-tight">Shipped Orders</h1>
                    <p className="text-[12px] text-gray-400 mt-0.5">Orders currently in transit or out for delivery</p>
                </div>
                {orders.length > 0 && (
                    <div className="flex items-center gap-2 bg-white border border-amber-100 px-4 py-2 rounded-xl shadow-sm">
                        <span className="w-2 h-2 rounded-full bg-amber-400" />
                        <span className="text-[12px] font-medium text-gray-600">
                            <span className="text-amber-600 font-semibold">{orders.length}</span> active shipments
                        </span>
                    </div>
                )}
            </div>
